...
class Document:
    ...
    def from_instances(cls, id: int, category: Category, topic: Topic, file_name: str):
        ...
        new_topic = topic.id
        return cls(id, new_category_id, new_topic, file_name)
    ...
    def __repr__(self):
        return f"Document {self.id}: {self.file_name}; category {self.category_id}, topic " \
               f"{self.topic_id}, tags: {', '.join([t for t in self.tags])}"
